...
[![rstudio mirror
downloads](https://cranlogs.r-pkg.org/badges/rnoaa?color=C9A115)](https://github.com/r-hub/cranlogs.app)
[![cran
version](https://www.r-pkg.org/badges/version/rnoaa)](https://cran.r-project.org/package=rnoaa)
...
NOAA has changed not only the base URL of the API but reorganized the
structure and endpoints of the data and API. The current `rnoaa` package
is still functional with the NOAA API v2 but the data available in the
API end at approximately 2022-09-15. A new NOAA weather package is
planned to replace rnoaa functionality where possible. At that point the
rnoaa package will be archived.
...
- NOAA NCDC climate data:
  ...
  - GHCN Daily data is available at
    <http://www.ncdc.noaa.gov/ghcn-daily-description> via FTP and HTTP
...
- GHCND FTP data (<ftp://ftp.ncdc.noaa.gov/pub/data/noaa>) - NOAA NCDC
  API has some/all (not sure really) of this data, but FTP allows to get
...
Documentation is at <https://docs.ropensci.org/rnoaa/>, and there are
many vignettes in the package itself, available in your R session, or on
CRAN (<https://cran.r-project.org/package=rnoaa>). The tutorials:
...
`ncdf4` is in Suggests in this package, meaning you only need `ncdf4` if
you are using any of the functions listed above. You’ll get an
...
<https://www.ncei.noaa.gov/access> for detailed info on each dataset. We
provide some information on the attributes in this package; see the
vignette for attributes
(<https://docs.ropensci.org/rnoaa/articles/ncdc_attributes.html>) to
find out more
...
- License: MIT
- Get citation information for `rnoaa` in R doing
