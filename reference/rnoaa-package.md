...
https://www.ncdc.noaa.gov/cdo-web/webservices/v2, all of which functions
start with `ncdc_`. An access token, or API key, is required to use all
the `ncdc_` functions. The key is required by NOAA, not us. Go to the
link given above to get an API key.

More NOAA data sources are being added through time. Data sources and
their function prefixes are:
...
- `cpc_prcp` - rainfall data from the NOAA Climate Prediction Center
  (CPC)
...
The following is a breakdown of the functions that fetch data by HTTP
vs. FTP - done this way as we've noticed that during the ealry 2019
border wall shutdown most FTP services were up, while those that were
...
HTTP services (whether service is/was up or down during early 2019
shutdown)
...
FTP services (whether service is/was up or down during early 2019
shutdown)
...
We've tried to whenever possible detect whether a service is error due
to a government shutdown and give a message saying so. If you know a
service is down that rnoaa interacts with but we don't fail well during
a shutdown let us know.
...
The `meteo` family of functions are prefixed with `meteo_` and provide a
