...
`rnoaa` is an R wrapper for many NOAA data types, including National
...
Note that the `value` column has strangely large numbers for temperature
measurements. By convention, `rnoaa` doesn’t do any conversion of values
from the APIs and some APIs use seemingly odd units.

You have two options here:

1.  Use the `add_units` parameter on
    [`ncdc()`](https://docs.ropensci.org/rnoaa/reference/ncdc.md) to
    have `rnoaa` attempt to look up the units. This is a good idea to
    try first.

2.  Consult the documentation for whiechever dataset you’re accessing.
    In this case, `GHCND` has a README
    (<https://www1.ncdc.noaa.gov/pub/data/ghcn/daily/readme.txt>) which
    indicates `TMAX` is measured in tenths of degrees Celcius.

### See a `data.frame` with units

As mentioned above, you can use the `add_units` parameter with
[`ncdc()`](https://docs.ropensci.org/rnoaa/reference/ncdc.md) to ask
`rnoaa` to attempt to look up units for whatever data you ask it to
return. Let’s ask `rnoaa` to add units to some precipitation (PRCP)
data:
...
From the above output, we can see that the units for `PRCP` values are
“mm_tenths” which means tenths of a millimeter. You won’t always be so
lucky and sometimes you will have to look up the documentation on your
own.
...
![plot of chunk six](../reference/figures/six-1.png)

plot of chunk six

Note that `PRCP` values are in units of tenths of a millimeter, as we
found out above.
...
![plot of chunk
ncdc-plot-zero](../reference/figures/ncdc-plot-zero-1.png)

plot of chunk ncdc-plot-zero
...
![plot of chunk ncdc-plot-0](../reference/figures/ncdc-plot-0-1.png)
...
![plot of chunk ncdc-plot-1](../reference/figures/ncdc-plot-1-1.png)
...
![plot of chunk ncdc-plot-2](../reference/figures/ncdc-plot-2-1.png)
...
Then plot - the default passing in the combined plot plots the data
together. In this case it looks kind of weird since a straight line
combines two distant dates.
...
![plot of chunk
ncdc-plot-line](../reference/figures/ncdc-plot-line-1.png)

plot of chunk ncdc-plot-line

But we can pass in each separately, which uses `facet_wrap` in `ggplot2`
to plot each set of data in its own panel.
...
![plot of chunk
ncdc-plot-panel](../reference/figures/ncdc-plot-panel-1.png)
