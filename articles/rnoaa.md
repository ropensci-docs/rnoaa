...
You’ll need GDAL (<https://gdal.org/>) installed first. You may want to
use GDAL \>= `0.9-1` since that version or later can read TopoJSON
...
Mavericks, so install the source version):
...
<https://www.ncdc.noaa.gov/cdo-web/token> to get one. *You can’t use
this package without an API key.*
...
1.  Use the `add_units` parameter on `ncdc` to have `rnoaa` attempt to
    look up the units. This is a good idea to try first.
...
![plot of chunk
unnamed-chunk-13](../reference/figures/unnamed-chunk-13-1.png)
...
![plot of chunk
unnamed-chunk-14](../reference/figures/unnamed-chunk-14-1.png)
...
simply gets **all the data**. So the call takes a while, but once done,
is fun to play with.
